class GameOverScene extends Phaser.Scene {
  ...
  init(data) {
    // Prefer playerScore (from GameScene), fallback to data.score if ever used elsewhere
    this.finalScore = (data && (data.playerScore ?? data.score)) ?? 0;
  }
  create() {
    ...
    this.add.text(cx + 200, cy - 150, `${this.finalScore}`, {
      fontSize: '72px', fill: '#ffffff', fontFamily, stroke: '#000', strokeThickness: 6
    }).setOrigin(0.5).setScrollFactor(0);

    this.add.text(cx - 50, cy - 150, "Your Score:", {
      fontSize: '72px', fill: '#ffffff', fontFamily, stroke: '#000', strokeThickness: 6
    }).setOrigin(0.5).setScrollFactor(0);
