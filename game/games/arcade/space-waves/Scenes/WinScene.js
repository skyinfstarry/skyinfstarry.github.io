class WinScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.image(0, 0, 'winbg').setOrigin(0).setScrollFactor(0);


    if (this.textures.exists('lvlbox')) this.add.image(cx, cy - 50, 'lvlbox').setScale(0.55, 0.4).setScrollFactor(0);
    ...
    const next = nextKey
      ? this.add.image(cx - 255, cy + 280, nextKey).setScrollFactor(0)
    ...
    const lvlReplay = replayKey
      ? this.add.image(cx + 255, cy + 280, replayKey).setScrollFactor(0)
