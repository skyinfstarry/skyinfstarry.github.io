...
export default class GamePlayScene extends Phaser.Scene {
  constructor() {
    ...
    this.cars = [];
    this.carColors = [0xff0000, 0x0000ff]; // left lanes red, right lanes blue (for obstacles/targets)
  ...
  create() {
    ...
    this.lives = mech.initialLives ?? 3;
    this.score = mech.initialScore ?? 0;
    this.timeRemaining = mech.timeLimit ?? 60;
    ...
    this.timeLabel = this.uiText.timeLabel;
    ...
    // Background (texture key must exist in images section of config)
    ...
    this.livesText = this.add.text(
      width / 2,
      80,
      `${this.uiText.livesLabel}: ${this.lives}`,
      {
        fontFamily: "Outfit",
        fontSize: "48px",
        color: "#020101ff"
    ...
    this.timerText = this.add.text(
      width - 90,
      80,
      `${this.uiText.timeLabel}: ${this.timeRemaining}`,
      {
        fontFamily: "Outfit",
        fontSize: "44px",
        color: "#000000ff"
    ...
    // DO NOT start timer/spawner or input yet
    this.gameStarted = false;
    this.gameOver = false;
  ...
  stopGameplay() {
    ...
    // stop lane-switch input (buttons still work)
  ...
  updateTimer() {
    ...
    if (this.timerText) {
      this.timerText.setText(`${this.uiText.timeLabel}: ${this.timeRemaining}`);
    }

    if (this.timeRemaining <= 0) {
      // Time finished -> WIN (you can change logic if needed)
      this.handleWin();
    }
  }
  ...
  spawnTarget(lane) {
    ...
    const target = this.targetGroup.create(x, -50, "top_view_fuel");
  ...
  moveCar(pointer) {
    ...
    const { width } = this.scale;
    const carToMove = Math.floor(pointer.x / (width / 2)); // 0 = left, 1 = right
    const index = Phaser.Math.Clamp(carToMove, 0, this.cars.length - 1);
    const car = this.cars[index];

    if (!car || !car.canMove) return;

    // Play drift SFX on tap (no loop)
    if (this.driftKey && this.cache.audio.exists(this.driftKey)) {
  ...
  onHitObstacle(car, obstacle) {
    ...
    this.lives -= 1;
    if (this.livesText) {
      this.livesText.setText(`${this.uiText.livesLabel}: ${this.lives}`);
    }
  ...
  onHitTarget(car, target) {
    ...
    // +10 score for each target collected
    this.score += 10;
    if (this.scoreText) {
  ...
  showStartOverlay() {
    ...
    const controlCar1 = this.add.image(width / 2 - 410, rowStartY, "top_view_fullvehicle");
    controlCar1.setScale(0.5);

    const controlCar2 = this.add.image(width / 2 - 220, rowStartY, "top_view_fullvehicle2");
